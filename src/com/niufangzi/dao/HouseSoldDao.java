...
public class HouseSoldDao {

	private static Logger logger = LogManager.getLogger();
	
	public static boolean AddHouse(HouseSold house)
	{
		...
		try {
			String strSQL = "insert into t_houseSold (chengshi, quyu1, quyu2, quyu3, quyu4, xiaoqu, houseId, fangwuhuxing, jianzhumianji, taoneimianji, "
	...
	public static HouseSold GetHouse(String houseId)
	{
		ConnPoolUtil connPool = ConnPoolUtil.getInstance();
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null; 
		HouseSold house = null;		
		try {
			String strSQL = "select * from t_houseSold where houseId=?";
			conn = connPool.getConnection();
			pstmt = conn.prepareStatement(strSQL);
	...
	public static int QueryTotalCount(String chengshi, String filterStr)
	{
		...
		try {
			String strSQL = " select count(*) as nTotal from t_houseSold where chengshi=? " + filterStr;
			conn = connPool.getConnection();
			pstmt = conn.prepareStatement(strSQL);
	...
	public static boolean QueryRecords(String chengshi, String filterStr, int nOrderField, int nOrderType, 
										PageInfo pi, List<HouseSold> list_sold)
	{
		ConnPoolUtil connPool = ConnPoolUtil.getInstance();
		Connection conn = null;
		PreparedStatement pstmt = null;
		...
		try {
			String strSQL = "select * from t_houseSold where chengshi=? " + filterStr + strOrder + " limit ?,? ";
			conn = connPool.getConnection();
			pstmt = conn.prepareStatement(strSQL);
			pstmt.setString(1, chengshi);
	...
	//查找表r_houseSold中已有数据的chengjiaoriqi最小值
	public static Date QueryMinDate(String city, OperationState state)
	{
		...
		ResultSet rs = null; 	
		try {
			String strSQL = "select min(chengjiaoriqi) as minDate from t_houseSold where chengshi=?";
			conn = connPool.getConnection();
			pstmt = conn.prepareStatement(strSQL);
	...
	//查找表r_houseSold中已有数据的chengjiaoriqi最大值
	public static Date QueryMaxDate(String city, OperationState state)
	{
		...
		ResultSet rs = null; 
		try {
			String strSQL = "select max(chengjiaoriqi) as maxDate from t_houseSold where chengshi=?";
			conn = connPool.getConnection();
			pstmt = conn.prepareStatement(strSQL);
			pstmt.setString(1, city);
			rs = pstmt.executeQuery();
