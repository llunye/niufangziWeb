...
//统计每个市的日挂牌均价
public class RptSellingDayDao {

	private static Logger logger = LogManager.getLogger();
	
	private static boolean AddSellingDay(Date sellingDate, String chengshi, int sCount, int sPrice)
	{
		...
		boolean res = true;
		try {
			String strSQL = "insert into r_SellingDay(sellingDate, chengshi, sCount, sPrice, createTime) values(?,?,?,?, now())";
	...
	//删除某个城市某个月的统计信息
	private static boolean DeleteSellingDays(String chengshi, Date startDate, Date endDate)
	{
		ConnPoolUtil connPool = ConnPoolUtil.getInstance();
		Connection conn = null;
		PreparedStatement pstmt = null;
		boolean res = true;
		try {
			String strSQL = "delete from r_SellingDay where chengshi=? and sellingDate>=? and sellingDate<=?";
			conn = connPool.getConnection();
			pstmt = conn.prepareStatement(strSQL);
											...
											List<Integer> list_count, List<Integer> list_price)
	{
		...
		list_price.clear();
		try {
			String strSQL = "select guapaishijian, count(*) as nCount, avg(guapaijiage/jianzhumianji) as nPrice from t_houseSelling "
							+ " where fangwuyongtu!=? and suozailouceng!=? and (guapaijiage/jianzhumianji)>? "
							+ " and chengshi=? and guapaishijian>=? and guapaishijian<=?  group by guapaishijian";	
            ...
            while (rs.next()) {
            	list_date.add(new Date(rs.getTimestamp("guapaishijian").getTime()));
	...
	public static Boolean SumDays(String chengshi, Date startDate, Date endDate)
	{
		...
		DeleteSellingDays(chengshi, startDate, endDate);
		for(int i=0; i<list_date.size(); i++)
		{
			AddSellingDay(list_date.get(i), chengshi, list_count.get(i), list_price.get(i));
		}
		return res;
	}
	...
	public static boolean QueryPriceRecords(String chengshi, int maxCount, PeriodType pType, List<Integer> list_price, List<String> list_date)
	{
		...
		list_date.clear();
		list_price.clear();
		String strTmp = CommonDao.GetSumDateFormat("sellingDate", pType);
		try {
			String strSQL = " select " + strTmp + " as strDate, avg(sPrice) as sumPrice from r_sellingDay where chengshi=? "
	...
	public static boolean QueryCountRecords(String chengshi, int maxCount, PeriodType pType, List<Integer> list_count, List<String> list_date)
	{
		...
		String strTmp = CommonDao.GetSumDateFormat("sellingDate", pType);
		try {
			String strSQL = " select " + strTmp + " as strDate, sum(sCount) as sumCount from r_sellingDay where chengshi=? "
							+ " group by strDate order by strDate desc limit ? ";
			conn = connPool.getConnection();
			pstmt = conn.prepareStatement(strSQL);
			pstmt.setString(1, chengshi);
	...
	//查找表r_sellingDay中已有数据的SellingDate最大值
	public static Date QueryMaxDate(String city, OperationState state)
	{
		...
		ResultSet rs = null; 	
		try {
			String strSQL = "select max(sellingDate) as maxDate from r_sellingDay where chengshi=?";
			conn = connPool.getConnection();
			pstmt = conn.prepareStatement(strSQL);
