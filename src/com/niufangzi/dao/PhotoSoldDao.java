...
public class PhotoSoldDao {

	private static Logger logger = LogManager.getLogger();
	
	public static int AddAPhoto(PhotoSold photo)
	{
		ConnPoolUtil connPool = ConnPoolUtil.getInstance();
		Connection conn = null;
		PreparedStatement pstmt = null;
		try {
			String strSQL = "insert into t_photoSold (chengshi, houseId, photoName, url, status, createTime, updateTime) values (?,?,?,?,?,?,?)";
	...
	public static int AddPhotos(List<PhotoSold> list_photo)
	{
		int i=0;
	...
	public static String GetPhotoURL(String city, String houseId, String photoName)
	{
		...
		String strURL = "";
		try {
			String strSQL = "select url from t_photoSold where chengshi=? and houseId=? and PhotoName=?";
			conn = connPool.getConnection();
			pstmt = conn.prepareStatement(strSQL);
