package com.niufangzi.model;

import java.util.Date;

//“在售”房屋
public class HouseSelling  {
	
	public String getChengshi() {
		return chengshi;
	}


	public void setChengshi(String chengshi) {
		this.chengshi = chengshi;
	}


	public String getQuyu1() {
		return quyu1;
	}


	public void setQuyu1(String quyu1) {
		this.quyu1 = quyu1;
	}


	public String getQuyu2() {
		return quyu2;
	}


	public void setQuyu2(String quyu2) {
		this.quyu2 = quyu2;
	}


	public String getQuyu3() {
		return quyu3;
	}


	public void setQuyu3(String quyu3) {
		this.quyu3 = quyu3;
	}


	public String getQuyu4() {
		return quyu4;
	}


	public void setQuyu4(String quyu4) {
		this.quyu4 = quyu4;
	}


	public String getXiaoqu() {
		return xiaoqu;
	}


	public void setXiaoqu(String xiaoqu) {
		this.xiaoqu = xiaoqu;
	}


	public String getHouseId() {
		return houseId;
	}


	public void setHouseId(String houseId) {
		this.houseId = houseId;
	}


	public String getFangwuhuxing() {
		return fangwuhuxing;
	}


	public void setFangwuhuxing(String fangwuhuxing) {
		this.fangwuhuxing = fangwuhuxing;
	}


	public float getJianzhumianji() {
		return jianzhumianji;
	}


	public void setJianzhumianji(float jianzhumianji) {
		this.jianzhumianji = jianzhumianji;
	}
	
	public float getTaoneimianji() {
		return taoneimianji;
	}

	public void setTaoneimianji(float taoneimianji) {
		this.taoneimianji = taoneimianji;
	}

	public String getFangwuchaoxiang() {
		return fangwuchaoxiang;
	}


	public void setFangwuchaoxiang(String fangwuchaoxiang) {
		this.fangwuchaoxiang = fangwuchaoxiang;
	}


	public String getZhuangxiuqingkuang() {
		return zhuangxiuqingkuang;
	}


	public void setZhuangxiuqingkuang(String zhuangxiuqingkuang) {
		this.zhuangxiuqingkuang = zhuangxiuqingkuang;
	}


	public String getGongnuanfangshi() {
		return gongnuanfangshi;
	}


	public void setGongnuanfangshi(String gongnuanfangshi) {
		this.gongnuanfangshi = gongnuanfangshi;
	}


	public int getChanquannianxian() {
		return chanquannianxian;
	}


	public void setChanquannianxian(int chanquannianxian) {
		this.chanquannianxian = chanquannianxian;
	}


	public String getSuozailouceng() {
		return suozailouceng;
	}


	public void setSuozailouceng(String suozailouceng) {
		this.suozailouceng = suozailouceng;
	}


	public int getZonglouceng() {
		return zonglouceng;
	}


	public void setZonglouceng(int zonglouceng) {
		this.zonglouceng = zonglouceng;
	}


	public String getJianzhuleixing() {
		return jianzhuleixing;
	}


	public void setJianzhuleixing(String jianzhuleixing) {
		this.jianzhuleixing = jianzhuleixing;
	}


	public int getJianchengniandai() {
		return jianchengniandai;
	}


	public void setJianchengniandai(int jianchengniandai) {
		this.jianchengniandai = jianchengniandai;
	}

	public String getHuxingjiegou() {
		return huxingjiegou;
	}
	
	public void setHuxingjiegou(String huxingjiegou) {
		this.huxingjiegou = huxingjiegou;
	}
	
	public String getJianzhujiegou() {
		return jianzhujiegou;
	}


	public void setJianzhujiegou(String jianzhujiegou) {
		this.jianzhujiegou = jianzhujiegou;
	}


	public String getTihubili() {
		return tihubili;
	}


	public void setTihubili(String tihubili) {
		this.tihubili = tihubili;
	}


	public String getPeibeidianti() {
		return peibeidianti;
	}


	public void setPeibeidianti(String peibeidianti) {
		this.peibeidianti = peibeidianti;
	}


	public Date getGuapaishijian() {
		return guapaishijian;
	}


	public void setGuapaishijian(Date guapaishijian) {
		this.guapaishijian = guapaishijian;
	}


	public Date getShangcijiaoyi() {
		return shangcijiaoyi;
	}


	public void setShangcijiaoyi(Date shangcijiaoyi) {
		this.shangcijiaoyi = shangcijiaoyi;
	}


	public String getFangwunianxian() {
		return fangwunianxian;
	}


	public void setFangwunianxian(String fangwunianxian) {
		this.fangwunianxian = fangwunianxian;
	}


	public String getDiyaxinxi() {
		return diyaxinxi;
	}


	public void setDiyaxinxi(String diyaxinxi) {
		this.diyaxinxi = diyaxinxi;
	}


	public String getJiaoyiquanshu() {
		return jiaoyiquanshu;
	}


	public void setJiaoyiquanshu(String jiaoyiquanshu) {
		this.jiaoyiquanshu = jiaoyiquanshu;
	}


	public String getFangwuyongtu() {
		return fangwuyongtu;
	}


	public void setFangwuyongtu(String fangwuyongtu) {
		this.fangwuyongtu = fangwuyongtu;
	}


	public String getChanquansuoshu() {
		return chanquansuoshu;
	}


	public void setChanquansuoshu(String chanquansuoshu) {
		this.chanquansuoshu = chanquansuoshu;
	}


	public String getFangbenbeijian() {
		return fangbenbeijian;
	}


	public void setFangbenbeijian(String fangbenbeijian) {
		this.fangbenbeijian = fangbenbeijian;
	}


	public String getZhuangtai() {
		return zhuangtai;
	}


	public void setZhuangtai(String zhuangtai) {
		this.zhuangtai = zhuangtai;
	}


	public float getGuapaijiage() {
		return guapaijiage;
	}


	public void setGuapaijiage(float guapaijiage) {
		this.guapaijiage = guapaijiage;
	}


	public float getChengjiaojiage() {
		return chengjiaojiage;
	}


	public void setChengjiaojiage(float chengjiaojiage) {
		this.chengjiaojiage = chengjiaojiage;
	}

	public Date getChengjiaoriqi() {
		return chengjiaoriqi;
	}


	public void setChengjiaoriqi(Date chengjiaoriqi) {
		this.chengjiaoriqi = chengjiaoriqi;
	}


	public String getChengjiaotujing() {
		return chengjiaotujing;
	}


	public void setChengjiaotujing(String chengjiaotujing) {
		this.chengjiaotujing = chengjiaotujing;
	}

	public int getLiulan() {
		return liulan;
	}


	public void setLiulan(int liulan) {
		this.liulan = liulan;
	}


	public int getGuanzhu() {
		return guanzhu;
	}


	public void setGuanzhu(int guanzhu) {
		this.guanzhu = guanzhu;
	}


	public int getDaikan() {
		return daikan;
	}


	public void setDaikan(int daikan) {
		this.daikan = daikan;
	}


	public int getTiaojia() {
		return tiaojia;
	}


	public void setTiaojia(int tiaojia) {
		this.tiaojia = tiaojia;
	}


	public int getZhouqi() {
		return zhouqi;
	}


	public void setZhouqi(int zhouqi) {
		this.zhouqi = zhouqi;
	}
	
	public Date getCreateTime() {
		return createTime;
	}


	public void setCreateTime(Date createTime) {
		this.createTime = createTime;
	}


	public Date getUpdateTime() {
		return updateTime;
	}


	public void setUpdateTime(Date updateTime) {
		this.updateTime = updateTime;
	}



	//地理位置
	private String chengshi;       //城市
	private String quyu1;          //区域1 (区)
	private String quyu2;          //区域2 (街道)
	private String quyu3;          //区域3
	private String quyu4;          //区域4
	private String xiaoqu;         //小区
	
	//经纪人信息

	//基础信息
	private String houseId;             //房屋编号
	private String fangwuhuxing;        //房屋户型
	private float  jianzhumianji;       //建筑面积
	private float  taoneimianji;        //套内面积
	private String fangwuchaoxiang;     //房屋朝向
	private String zhuangxiuqingkuang;  //装修情况
	private String gongnuanfangshi;     //供暖方式
	private int    chanquannianxian;    //产权年限        
	private String suozailouceng ;      //所在楼层
	private int    zonglouceng;         //总楼层
	private String huxingjiegou;        //户型结构
	private String jianzhuleixing;      //建筑类型
	private int    jianchengniandai;    //建成年代
	private String jianzhujiegou;       //建筑结构
	private String tihubili;            //梯户比例
	private String peibeidianti;        //配备电梯
	
	private Date   guapaishijian;       //挂牌时间
	private Date   shangcijiaoyi;       //上次交易
	private String fangwunianxian;      //房屋年限
	private String diyaxinxi;           //抵押信息  
	private String jiaoyiquanshu;       //交易权属
	private String fangwuyongtu;        //房屋用途
	private String chanquansuoshu;      //产权所属
	private String fangbenbeijian;      //房本备件
	
	//概述
	private String zhuangtai;           //状态̬
	private float  guapaijiage;         //挂牌价格
	private float  chengjiaojiage;      //成交价格
	private Date   chengjiaoriqi;       //成交日期
	private String chengjiaotujing;     //成交途径
	private int    liulan;              //浏览
	private int    guanzhu;             //关注
	private int    daikan;              //带看
	private int    tiaojia;             //调价
	private int    zhouqi;              //(成交)周期
	
	private Date createTime;        //创建时间
	private Date updateTime;        //更新时间
	

	public HouseSelling()  //构造函数
	{
		jianzhumianji = 0;
		taoneimianji = 0;
		chanquannianxian = 0;        
		zonglouceng = 0;
		jianchengniandai = 0;
		guapaijiage = 0;
		chengjiaojiage = 0;
		guapaishijian = new Date();
		chengjiaoriqi = new Date();
		shangcijiaoyi = new Date();
		createTime = new Date();
		updateTime = new Date();
	}
	
	
	
}
